...
static bool _try_convert_hal_to_camx_pixel_format(uint32_t hal_format, uint32_t gralloc_flags,
                                                   CamxPixelFormat& camx_format) {
    switch (hal_format) {
        case HAL_PIXEL_FORMAT_BLOB:
            camx_format = CamxPixelFormat::CAMERA_PIXEL_FORMAT_BLOB;
            return true;
        case HAL_PIXEL_FORMAT_RAW10:
            camx_format = CamxPixelFormat::CAMERA_PIXEL_FORMAT_RAW10;
            return true;
        case HAL_PIXEL_FORMAT_RAW12:
            camx_format = CamxPixelFormat::CAMERA_PIXEL_FORMAT_RAW12;
            return true;
        case HAL_PIXEL_FORMAT_RAW16:
            camx_format = CamxPixelFormat::CAMERA_PIXEL_FORMAT_RAW16;
            return true;
        case HAL_PIXEL_FORMAT_YCBCR_420_888:
            if (gralloc_flags & GRALLOC_USAGE_HW_VIDEO_ENCODER) {
                camx_format = CamxPixelFormat::CAMERA_PIXEL_FORMAT_NV12_VENUS;
            } else {
                camx_format = CamxPixelFormat::CAMERA_PIXEL_FORMAT_NV21_ZSL;
            }
            return true;
        default:
            return false;
...
static int _calculate_camx_format_params(uint32_t hal_format, uint32_t gralloc_flags, int width, int height,
                                       int* stride, int* scanline, unsigned int* size) {
    CamxPixelFormat cam_format;
    int res;

    // Try to convert HAL format to CamX format
    if (!_try_convert_hal_to_camx_pixel_format(hal_format, gralloc_flags, cam_format)) {
...
int allocate_one_buffer(mpa_ion_buf_t* buf, int width, int height, uint32_t hal3_format, uint32_t gralloc_flags) {
    ...
    * that ourselves.
    */
    res = _calculate_camx_format_params(hal3_format, gralloc_flags, width, height, &stride, &scanline, &size);
    if (res != 0) {
